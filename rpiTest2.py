...
def txVideo(IP, PORT):
	...
	while isFirstConn == False:
		...
		if isFirstConn == False:
			tTmp = time.time()
			_, img = webcam.read()
	        	print 'Read: ' + str(time.time() - tTmp)

			tTmp = time.time()
			image_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
			data = pygame.image.frombuffer(image_rgb.tostring(), image_rgb.shape[1::-1], "RGB")
			print 'Conv: ' + str(time.time() - tTmp)
			
			tTmp = time.time()
			pImg = pygame.image.tostring(data, "RGB")
			conn.sendall(pImg)
			print 'Send: ' + str(time.time() - tTmp)

			print 'Sent ' + str(sys.getsizeof(pImg)) + 'bytes.'
